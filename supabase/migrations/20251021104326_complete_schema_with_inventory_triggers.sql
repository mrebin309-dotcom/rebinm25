/*
  # Complete Inventory Management System Schema with Automatic Stock Management

  ## Overview
  This migration creates the complete inventory management database schema with automatic
  inventory tracking triggers. When a sale is made, product stock is automatically decreased.
  When a return is approved, stock is automatically increased.
...
RETURNS TRIGGER AS $$
BEGIN
  -- Only update stock for actual products (not services)
  ...
  IF NEW.product_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
    ...
    AND stock >= NEW.quantity; -- Only update if enough stock
  END IF;
...
RETURNS TRIGGER AS $$
BEGIN
  -- Only update stock for approved returns and actual products (not services)
  -- Check if product_id is a valid UUID
...
-- Trigger to decrease stock after a sale is inserted
...
-- Trigger to increase stock after a return is inserted with approved status
